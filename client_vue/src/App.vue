<template>
  <div id="app">
    ...
    <router-view  :isFilter="is_filter"/>
     </div>
            ...
            .then(function (response) {
              ...
              var data = response.data
                ...
                if (localStorage['filter_cars']) {
                  ...
                  localStorage['filter_cars'] = JSON.stringify(data.cars)
                }
  ...
  methods:{
    changeSelect(){
          ...
          if(this.year_to == "-" || this.year_from == "-") this.err_year = "this parameter is required"
              ...
              //axios.get("http://localhost:88/REST/client/api/cars/"+req_str, this.config)
               axios.get('http://192.168.0.15/~user15/REST/client/api/cars/'+req_str, this.config)
                .then(function (response) {
                    localStorage['filter_cars'] = JSON.stringify(response.data)
        ...
        clearFilter(){
            ...
            this.fullInfoCarRedirect = true 
            localStorage['filter_cars'] = JSON.stringify(this.carsAll)
            if(this.is_filter == true)this.is_filter=false
                    else this.is_filter=true
        }
  }
}
</script>
